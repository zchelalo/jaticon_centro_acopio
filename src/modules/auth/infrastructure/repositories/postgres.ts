...
export class PostgresRepository implements AuthRepository {
   ...
   * ```
  */
  async getTokenByValue(tokenValue: string): Promise<TokenValue> {
  ...
  */
   ...
   * ```
  */
  async revokeTokenByValue(tokenValue: string): Promise<void> {
    await this.getTokenByValue(tokenValue)
